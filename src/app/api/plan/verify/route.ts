  ...
  try {
    ...
    try {
      ...
      if (membershipsResponse.ok) {
        ...
        console.warn('[Plan Verify] Failed to fetch memberships:', membershipsResponse.status, await membershipsResponse.text().catch(() => ''))
      }
      
      // Determine correct plan based on Whop API
      // If no active memberships found, user should be 'free'
      const correctPlan = hasActiveProMembership ? 'pro' : 'free'
