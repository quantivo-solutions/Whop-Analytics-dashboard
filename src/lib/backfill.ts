...
export async function performBackfill(
  ...
  console.log(`[Bootstrap] 🚀 Starting historical data ingestion for company ${companyId} for the last ${days} days...`)
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    try {
      const progress = daysWritten + 1
      const progressPercent = Math.round((progress / days) * 100)
      console.log(`[Bootstrap] Processing ${dateStr} (${progress}/${days} days, ${progressPercent}%)...`)
      ...
      if (hasData) {
        console.log(`[Bootstrap]   ✅ ${dateStr}: $${summary.grossRevenue.toFixed(2)}, ${summary.activeMembers} active, ${summary.newMembers} new`)
      } else {
        console.log(`[Bootstrap]   ✅ ${dateStr}: No activity (all zeros)`)
      }

      // Rate-limit calls to avoid Whop throttling (200ms between requests for safety)
      await new Promise(resolve => setTimeout(resolve, 200))
    } catch (error) {
      console.error(`[Bootstrap]   ❌ Error processing ${dateStr}:`, error)
      // Continue processing remaining days even if one fails
    }
  }

  console.log(`[Bootstrap] ✅ Historical data ingestion complete for company ${companyId}. Wrote ${daysWritten} out of ${days} days.`)
